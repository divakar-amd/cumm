...
public:
  CUDAEventCore() { checkCudaErrors(hipEventCreate(&event_)); }
  ...
  void record(hipStream_t stream = nullptr) {
    checkCudaErrors(hipEventRecord(event_, stream));
  }

  void stream_wait_me(hipStream_t stream, int flag = 0) {
    checkCudaErrors(hipStreamWaitEvent(stream, event_, flag));
  }

  void sync() {
    checkCudaErrors(hipEventSynchronize(event_));
  }

  hipEvent_t get_event() { return event_; }
};
...
public:
  ...
  CUDAEvent& record(std::uintptr_t stream = 0) {
    ...
    event_->record(reinterpret_cast<hipStream_t>(stream));
    return *this;
  }

  CUDAEvent& stream_wait_me(std::uintptr_t stream, int flag = 0) {
    TV_ASSERT_RT_ERR(event_, "event is empty");
    event_->stream_wait_me(reinterpret_cast<hipStream_t>(stream), flag);
    return *this;
  }
  ...
  static float duration(CUDAEvent start, CUDAEvent stop) {
    ...
    checkCudaErrors(hipEventElapsedTime(&ms, start.event_->get_event(),
...
#ifdef TV_CUDA
  CUDAKernelTimerGuard(std::string name, CUDAKernelTimer timer,
                       hipStream_t stream = nullptr, bool print_exit = false): CUDAKernelTimerGuard(name, timer, reinterpret_cast<std::uintptr_t>(stream), print_exit){}
#endif
...
#ifdef TV_CUDA
inline auto measure_and_print_guard(std::string name, hipStream_t stream = nullptr){
