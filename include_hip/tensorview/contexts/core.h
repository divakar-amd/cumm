...
public:
  ContextCore() {
    ...
    stream_mgr.creater = []() -> std::uintptr_t {
      hipStream_t stream_cuda;
      checkCudaErrors(hipStreamCreate(&stream_cuda));
      return reinterpret_cast<std::uintptr_t>(stream_cuda);
    };
    stream_mgr.deleter = [](std::uintptr_t ptr_int) {
      hipStreamDestroy(reinterpret_cast<hipStream_t>(ptr_int));
    };
...
#ifdef TV_CUDA
  Context &set_cuda_stream(hipStream_t stream) {
    check_ptr_valid();
...
#ifdef TV_CUDA
    check_ptr_valid();
    checkCudaErrors(hipStreamSynchronize(cuda_stream()));
#endif
  }
