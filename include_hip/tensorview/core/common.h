...
#include "printf2.h"
    ...
    (!defined(__clang__) && defined(TORCH_HIP_VERSION) && TORCH_HIP_VERSION >= 11000)
...
#define TV_CHECK_CUDA_ERR()                                                    \
  {                                                                            \
    auto __macro_err = hipGetLastError();                                     \
    if (__macro_err != hipSuccess) {                                          \
      std::stringstream __macro_s;                                             \
      __macro_s << __FILE__ << ":" << __LINE__ << "\n";                        \
...
#define TV_CHECK_CUDA_ERR_V2(...)                                              \
  {                                                                            \
    ...
    if (__macro_err != hipSuccess) {                                          \
      ...
      __macro_s << __FILE__ << ":" << __LINE__ << "\n";                        \
      __macro_s << "cuda execution failed with error " << __macro_err;         \
      __macro_s << " " << hipGetErrorString(__macro_err) << "\n";             \
      tv::sstream_print(__macro_s, __VA_ARGS__);                               \
      TV_BACKTRACE_PRINT(__macro_s);                                           \
      throw std::runtime_error(__macro_s.str());                               \
    }                                                                          \
...
#define TV_CUDART_RESULT_CHECK(EXPR)                                           \
  do {                                                                         \
    hipError_t __macro_err = EXPR;                                            \
    if (__macro_err != hipSuccess) {                                          \
      auto error_unused = hipGetLastError();                                  \
      std::stringstream __macro_s;                                             \
...
#define TV_CUDADRV_RESULT_CHECK(EXPR)                                          \
  do {                                                                         \
    hipError_t __macro_err = EXPR;                                               \
    if (__macro_err != hipError_t::hipSuccess) {                               \
      const char *errstr;                                                      \
      hipDrvGetErrorString(__macro_err, &errstr);                                  \
      std::stringstream __macro_s;                                             \
      __macro_s << __func__ << " " << __FILE__ << ":" << __LINE__ << "\n";     \
      __macro_s << "cuda failed with error " << __macro_err;                   \
      __macro_s << " " << errstr << "\n";                                      \
