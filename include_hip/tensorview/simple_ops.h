...
void transform(hipStream_t stream, tv::Tensor out, tv::Tensor a,
               tv::Tensor b) {
  ...
  tv::Dispatch<all_t>()(a.dtype(), [&](auto TAValue) {
    ...
    auto ctx = thrust::hip::par.on(stream);
    thrust::transform(ctx, ptr_a, ptr_a + a.size(), ptr_b, ptr_o, op);
  });
}

template <template <class> class OpClass, typename T = void>
tv::Tensor transform(hipStream_t stream, tv::Tensor a, tv::Tensor b) {
...
template <template <class> class OpClass, typename T = void>
void unary(hipStream_t stream, tv::Tensor out, tv::Tensor a) {
...
template <template <class> class OpClass, typename T = void>
tv::Tensor reduce(hipStream_t stream, tv::Tensor a, tv::Tensor init) {
  using all_t = std::conditional_t<std::is_same<T, void>::value,
                                   tv::detail::all_tensor_types_t, mp_list<T>>;
...
template <template <class> class OpClass, typename T = void>
tv::Tensor reduce(hipStream_t stream, tv::Tensor a) {
