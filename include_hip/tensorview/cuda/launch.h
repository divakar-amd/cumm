...
template <size_t NumThreads = 1024> struct LaunchEx {
  LaunchEx(dim3 blocks_, dim3 threads_, hipStream_t stream_ = 0)
  ...
  LaunchEx(int64_t size, hipStream_t stream_ = 0)
  ...
  template <typename F, class... Args> void operator()(F &&f, Args &&...args) {
    std::forward<FhipLaunchKernelGGL((>(f)), dim3(blocks), dim3(threads), smem_size, stream, 
        std::forward<Args>(args)...);
  }

  template <class... Args>
  void run_launch_api(hipFunction_t kernel, Args &&...args) {
    void *args_vec[] = {&args...};
  ...
  template <class... Args>
  void run_launch_driver_api(hipFunction_t kernel, Args &&...args) {
    void *args_vec[] = {&args...};
    TV_CUDA_RESULT_CHECK(
        hipModuleLaunchKernel(kernel, blocks.x, blocks.y, blocks.z, // grid dim
                       threads.x, threads.y, threads.z,      // block dim
                       smem_size, stream, // shared mem and stream
                       args_vec, 0));     // arguments
  }
