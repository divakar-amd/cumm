...
public:
  ...
  NVRTCProgram(std::string code,
    ...
    const char **header_name_ptr = nullptr;
    if (headers_.size() > 0) {
    ...
    for (size_t i = 0; i < name_exprs_.size(); ++i) {
      TV_NVRTC_SAFE_CALL(hiprtcAddNameExpression(prog_, name_exprs_[i].c_str()));
    }

    hiprtcResult compileResult =
        hiprtcCompileProgram(prog_,             // prog
                            opts.size(),       // numOptions
                            opts_ptrs.data()); // options

    size_t logSize;
    TV_NVRTC_SAFE_CALL(hiprtcGetProgramLogSize(prog_, &logSize));
    std::string log(logSize, '0');
    auto nvrtc_compile_res = hiprtcGetProgramLog(prog_, &log[0]);
    if (compileResult != HIPRTC_SUCCESS) {
      tv::ssprint(log);
    }
    TV_NVRTC_SAFE_CALL(nvrtc_compile_res);
    compile_log_ = log;
    TV_ASSERT_RT_ERR(compileResult == HIPRTC_SUCCESS, "nvrtc compile failed.");
    // post check
  ...
  std::string get_lowered_name(std::string name) const {
    ...
    TV_NVRTC_SAFE_CALL(hiprtcGetLoweredName(prog_,
...
public:
  ...
  NVRTCModule &load() {
    ...
    if (!cudadevrt_path_.empty()) {
      ...
      TV_CUDA_RESULT_CHECK(wrapper_.cuDrvLinkAddFile(
          linkState_, HIPRTC_JIT_INPUT_LIBRARY, cudadevrt_path_.c_str(), 0, 0, 0));
      TV_CUDA_RESULT_CHECK(
          wrapper_.cuDrvLinkAddData(linkState_, HIPRTC_JIT_INPUT_PTX, &ptx[0],
  ...
  hipFunction_t kernel(std::string name) {

    TV_ASSERT_RT_ERR(module_ != nullptr, "moculde must be loaded");
    hipFunction_t k = nullptr;
    TV_CUDA_RESULT_CHECK(
  ...
  std::unordered_map<std::string, int> get_kernel_attributes(std::string name) {
    ...
    TV_CUDA_RESULT_CHECK(wrapper_.cuDrvFuncGetAttribute(
        &pi, HIP_FUNC_ATTRIBUTE_MAX_THREADS_PER_BLOCK, k));
    res["max_threads_per_block"] = pi;
    TV_CUDA_RESULT_CHECK(wrapper_.cuDrvFuncGetAttribute(
        &pi, HIP_FUNC_ATTRIBUTE_SHARED_SIZE_BYTES, k));
    res["shared_size_bytes"] = pi;
    TV_CUDA_RESULT_CHECK(wrapper_.cuDrvFuncGetAttribute(
        &pi, HIP_FUNC_ATTRIBUTE_CONST_SIZE_BYTES, k));
    res["const_size_bytes"] = pi;
    TV_CUDA_RESULT_CHECK(wrapper_.cuDrvFuncGetAttribute(
        &pi, HIP_FUNC_ATTRIBUTE_LOCAL_SIZE_BYTES, k));
    res["local_size_bytes"] = pi;
    TV_CUDA_RESULT_CHECK(
        wrapper_.cuDrvFuncGetAttribute(&pi, HIP_FUNC_ATTRIBUTE_NUM_REGS, k));
    res["num_regs"] = pi;
    TV_CUDA_RESULT_CHECK(
        wrapper_.cuDrvFuncGetAttribute(&pi, HIP_FUNC_ATTRIBUTE_PTX_VERSION, k));
    res["ptx_version"] = pi;
    TV_CUDA_RESULT_CHECK(wrapper_.cuDrvFuncGetAttribute(
        &pi, HIP_FUNC_ATTRIBUTE_BINARY_VERSION, k));
    res["binary_version"] = pi;
    TV_CUDA_RESULT_CHECK(wrapper_.cuDrvFuncGetAttribute(
        &pi, HIP_FUNC_ATTRIBUTE_MAX_DYNAMIC_SHARED_SIZE_BYTES, k));
    res["max_dynamic_shared_size_bytes"] = pi;
    TV_CUDA_RESULT_CHECK(wrapper_.cuDrvFuncGetAttribute(
        &pi, HIP_FUNC_ATTRIBUTE_PREFERRED_SHARED_MEMORY_CARVEOUT, k));
  ...
  void set_max_dynamic_shared_size_bytes(std::string name, int size) {
    ...
    TV_CUDA_RESULT_CHECK_V2(wrapper_.cuDrvFuncSetAttribute(
        k, HIP_FUNC_ATTRIBUTE_MAX_DYNAMIC_SHARED_SIZE_BYTES, size), 
  ...
  void set_preferred_smem_carveout(std::string name, int carveout) {
    ...
    auto k = kernel(name);
    TV_CUDA_RESULT_CHECK_V2(wrapper_.cuDrvFuncSetAttribute(
        k, HIP_FUNC_ATTRIBUTE_PREFERRED_SHARED_MEMORY_CARVEOUT, carveout), 
  ...
  void run_kernel(std::string name, std::array<int, 3> blocks,
    ...
    if (smem_size >= (48 << 10)) {
      TV_CUDA_RESULT_CHECK_V2(wrapper_.cuDrvFuncSetAttribute(
          k, HIP_FUNC_ATTRIBUTE_MAX_DYNAMIC_SHARED_SIZE_BYTES, smem_size), smem_size, get_kernel_attributes(name));
      TV_CUDA_RESULT_CHECK_V2(wrapper_.cuDrvFuncSetAttribute(
          k, HIP_FUNC_ATTRIBUTE_PREFERRED_SHARED_MEMORY_CARVEOUT, 100), "hipFuncAttributePreferredSharedMemoryCarveout");
    }
...
private:
  ...
  hiprtcLinkState linkState_ = nullptr;
#else
