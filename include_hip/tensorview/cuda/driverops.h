...
template <typename T>
void host2dev(T *dst, const T *src, size_t size, hipStream_t s) {
  TV_CUDART_RESULT_CHECK(
      hipMemcpyAsync(dst, src, size * sizeof(T), hipMemcpyHostToDevice, s));
}
...
template <typename T>
void dev2host(T *dst, const T *src, size_t size, hipStream_t s) {
  TV_CUDART_RESULT_CHECK(
      hipMemcpyAsync(dst, src, size * sizeof(T), hipMemcpyDeviceToHost, s));
}
...
template <typename T>
void dev2dev(T *dst, const T *src, size_t size, hipStream_t s) {
  TV_CUDART_RESULT_CHECK(
      hipMemcpyAsync(dst, src, size * sizeof(T), hipMemcpyDeviceToDevice, s));
}
...
template <typename T> void dev2dev(T *dst, const T *src, size_t size) {
  TV_CUDART_RESULT_CHECK(
      hipMemcpy(dst, src, size * sizeof(T), hipMemcpyDeviceToDevice));
}
...
template <typename T>
void host2host(T *dst, const T *src, size_t size, hipStream_t s) {
  TV_CUDART_RESULT_CHECK(
      hipMemcpyAsync(dst, src, size * sizeof(T), hipMemcpyHostToHost, s));
}
...
template <typename T> void host2host(T *dst, const T *src, size_t size) {
  TV_CUDART_RESULT_CHECK(
      hipMemcpy(dst, src, size * sizeof(T), hipMemcpyHostToHost));
}
...
void zero_dev(TensorView<T, Rank, PtrTraits, Tindex> tensor) {
  TV_CUDART_RESULT_CHECK(
      hipMemset(tensor.data(), 0, tensor.size() * sizeof(T)));
}

template <typename T, int Rank, template <class> class PtrTraits,
          typename Tindex>
void zero_dev(TensorView<T, Rank, PtrTraits, Tindex> tensor, hipStream_t s) {
  TV_CUDART_RESULT_CHECK(
      hipMemsetAsync(tensor.data(), 0, tensor.size() * sizeof(T), s));
}
template <typename T, int Rank, template <class> class PtrTraits,
          typename Tindex>
...
struct FillDev<float, Rank, PtrTraits, Tindex> {
  static void run_async(TensorView<float, Rank, PtrTraits, Tindex> tensor,
                        float val, hipStream_t s) {
    detail::FloatBits fb;
    fb.src = val;
    TV_CUDADRV_RESULT_CHECK(
        hipMemsetAsync(reinterpret_cast<hipDeviceptr_t>(tensor.data()), fb.dst,
                         tensor.size(), s));
  }
  static void run(TensorView<float, Rank, PtrTraits, Tindex> tensor,
    ...
    TV_CUDADRV_RESULT_CHECK(cuMemsetD32(
        reinterpret_cast<hipDeviceptr_t>(tensor.data()), fb.dst, tensor.size()));
  }
};
...
struct FillDev<__half, Rank, PtrTraits, Tindex> {
  static void run_async(TensorView<__half, Rank, PtrTraits, Tindex> tensor,
                        __half val, hipStream_t s) {
    detail::Float16Bits fb;
    fb.src = val;
    TV_CUDADRV_RESULT_CHECK(
        hipMemsetD16Async(reinterpret_cast<hipDeviceptr_t>(tensor.data()), fb.dst,
                         tensor.size(), s));
  }
...
struct FillDev<uint32_t, Rank, PtrTraits, Tindex> {
  static void run_async(TensorView<uint32_t, Rank, PtrTraits, Tindex> tensor,
                        uint32_t val, hipStream_t s) {
    TV_CUDADRV_RESULT_CHECK(hipMemsetAsync(
        reinterpret_cast<hipDeviceptr_t>(tensor.data()), val, tensor.size(), s));
  }
  static void run(TensorView<uint32_t, Rank, PtrTraits, Tindex> tensor,
                  uint32_t val) {
    TV_CUDADRV_RESULT_CHECK(cuMemsetD32(
        reinterpret_cast<hipDeviceptr_t>(tensor.data()), val, tensor.size()));
  }
};
...
struct FillDev<uint16_t, Rank, PtrTraits, Tindex> {
  static void run_async(TensorView<uint16_t, Rank, PtrTraits, Tindex> tensor,
                        uint16_t val, hipStream_t s) {
    TV_CUDADRV_RESULT_CHECK(hipMemsetD16Async(
        reinterpret_cast<hipDeviceptr_t>(tensor.data()), val, tensor.size(), s));
  }
...
struct FillDev<uint8_t, Rank, PtrTraits, Tindex> {
  static void run_async(TensorView<uint8_t, Rank, PtrTraits, Tindex> tensor,
                        uint8_t val, hipStream_t s) {
    TV_CUDADRV_RESULT_CHECK(hipMemsetD8Async(
        reinterpret_cast<hipDeviceptr_t>(tensor.data()), val, tensor.size(), s));
  }
...
struct FillDev<int8_t, Rank, PtrTraits, Tindex> {
  static void run_async(TensorView<int8_t, Rank, PtrTraits, Tindex> tensor,
                        int8_t val, hipStream_t s) {
    detail::Int8Bits fb;
    fb.src = val;
    TV_CUDADRV_RESULT_CHECK(
        hipMemsetD8Async(reinterpret_cast<hipDeviceptr_t>(tensor.data()), fb.dst,
                        tensor.size(), s));
  }
