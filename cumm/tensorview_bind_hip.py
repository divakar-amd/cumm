...
from cumm.common import PyBind11, TensorView, TensorViewCPU, get_hip_version
...
class TensorViewBind(pccm.Class, pccm.pybind.PybindClassMixin):
    def __init__(self):
        ...
        if not CUMM_CPU_ONLY_BUILD:
            ...
            cuda_ver = get_hip_version().split(".")
            cuda_ver_ints = list(map(int, cuda_ver))
            self.build_meta.add_libraries("hiprtc")
            if compat.InLinux:
                self.build_meta.add_ldflags("g++", "-Wl,--no-as-needed", "-lhiprtc-builtins")
                self.build_meta.add_ldflags("clang++", "-Wl,--no-as-needed", "-lhiprtc-builtins")
                self.build_meta.add_ldflags("hipcc", "-Wl,--no-as-needed", "-lhiprtc-builtins")
            if not compat.InWindows:
...
#if !defined(TV_CUDA) || (TORCH_HIP_VERSION < 11040)
#include <cxxabi.h>
#endif
            """
  ...
  m.def("get_compute_capability", [](int index){
    if (index == -1){
      checkCudaErrors(hipGetDevice(&index));
    }
#ifdef TV_CUDA
    hipDeviceProp_t prop;
    checkCudaErrors(hipGetDeviceProperties(&prop, index));
